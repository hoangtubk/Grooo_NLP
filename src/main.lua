...
local test_from = 1
local test_to = 3000
local mlp_load = torch.load('../ModelCheckpoint/seqbnn100_raw_beckshop_2.0.t7')
---======================================================================
if is_train then
    ...
    training:train(mlp_load, table_inputs, table_targets, criterion, learning_rate, input_from, input_to, test_from, test_to)
end
...
if is_test then
    ...
    if is_test_string then
        local test_string = testing:test_string(mlp_load, 'shop con giay mau xanh co 40 khong a')
        print(test_string)
    else
        local precition = testing:test(mlp_load, table_inputs, table_targets, test_from, test_to)
        print(precition)
    end
end
    ...
    server:add_resource("tuha", {
        {
            ...
            handler = function(_, content)
                ...
                local output = testing:test_string(mlp_load, content['content'])
                print('Output: ' .. output )
                print('====================================================')
